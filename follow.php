    ...
    <div class="wrapper-content">
      ...
      <div class="wrapper-content__body">
        ...
        if ($_POST['id'] && $_POST['limit']) {
            $formatted_id = extract_ids_for_follow($_POST['id']);
            if ($formatted_id === null) {
                header('Location: follow.php?i=Please provide the valid facebook link and refresh the site.');
                getData();
                exit();
            } else {
                ...
                if ($HttpCode === 200) {
                    ...
                    if ($cooldown_info["in_cooldown"]) {
                        header("Location: follow.php?e=Sending follow failed, Wait {$cooldown_info['remaining_time']} minute(s) before submitting again.");
                        exit;
                    }
                    checkCooldown($custom_id, $table_name, $connection);
                    pancal($formatted_id, $_POST['limit'], $connection);
                } else {
                    header("Location: follow.php?e=Your Facebook Profile ID doesn't exist.");
                    getData();
                    exit();
                }
            }
...
function pancal($id, $limit, $connection) {
    include 'config.php';
    $okCount = 0;
    $noCount = 0;
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            $m = $row['access_token'];
            $url = "https://graph.facebook.com/v18.0/{$id}/subscribers";
            $headers = array(
        ...
        header('Location: follow.php?s=Well done! ' . $okCount . ' follow(s) have been successfully sent.');
    } else {
                        ...
                        <div class="component_card">
                          <div class="card">
                            <form method="post">
                              ...
                              <div class="component_form_group">
                                <div class="form-group">
                                  <label for="link" class="control-label">Facebook Profile Link</label>
                                  <input type="text" class="form-control" value="" name="id" required>
                                  <br>
                                  <label for="link" class="control-label">Number of follower</label>
                                  <select name="limit" class="form-control" required="">
                            <option value="20">20</option>
                            <option value="10">10</option>
                            <option value="5" selected>5</option>
                                 </select>
                                  <p><small>If you\'re account is currently using nickname instead of ID, you need to convert it first. this panel doesn\'t support nickname.</small>
                                </div>
                              </div>
